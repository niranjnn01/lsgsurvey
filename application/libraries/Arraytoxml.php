...
class ArrayToXml { 
     ...
     * @access Public 
     */ 
    function outputXML($bEcho = false,$xmlHead = true) { 
        $this->generateXML($xmlHead); 

        if (!$bEcho) { 
            return $this->XML; 
        } else { // End of if ($method == 'return') 
    ...
    function strToXMLKeySafe($str) { 
    	...
    	//var_dump($str);exit;
    	//var_dump( is_numeric( substr($str,0,1) ) );exit;
        if (is_numeric($str)) { 
